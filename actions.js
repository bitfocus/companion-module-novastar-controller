	...
	if (instance.config.modelID == 'vx1000') {
		actions['update_layer_vx1000'] = {
			...
			callback: async (event) => {
				...
				initialX = Math.max(0, Math.min(initialX, Math.pow(2, 32) - 1))

				initialY = Math.trunc(initialY)
				initialY = Math.max(0, Math.min(initialY, Math.pow(2, 32) - 1))

				hWidth = Math.trunc(hWidth)
				hWidth = Math.max(0, Math.min(hWidth, Math.pow(2, 32) - 1))

				vHeight = Math.trunc(vHeight)
				vHeight = Math.max(0, Math.min(vHeight, Math.pow(2, 32) - 1))
